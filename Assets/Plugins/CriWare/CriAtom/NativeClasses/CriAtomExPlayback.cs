...
public struct CriAtomExPlayback
{
	 ...
	 * 再生中のキューのトラック情報を取得するための構造体です。<br/>
	 * \sa CriAtomExPlayback::GetTrackInfo
	 */
	 ...
	 * \par 説明:
	 * ::CriAtomExPlayer::Start 関数で再生されたキューのトラック情報を取得します。<br/>
	 ...
	 * - 再生中のキューがポリフォニックタイプ、またはセレクタ参照のスイッチタイプである。（トラック情報が複数存在する可能性があるため）<br/>
	 ...
	 * <returns>取得に成功したか</returns>
	 * \par 説明:
	 * ::CriAtomExPlayer::Start 関数で再生されたキューのビート同期情報を取得します。<br/>
